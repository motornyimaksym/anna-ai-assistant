...
The private assistant uses OpenAI Responses function calls. Set a Responses-capable `OPENAI_MODEL` and a funded API key. Jev is an optional precheck: `JEV_TOKEN` stays backend-only, and local `.env` is not a deployed secret. The function declares `JEV_TOKEN` as a Firebase secret binding; provision that secret to use Jev in production. A missing runtime token or failed Jev decision proceeds through the regular OpenAI flow; only a valid score reaching the threshold holds the turn for human review. Both Business messages and private bot DMs are supported; include `message` in Telegram webhook `allowed_updates` for DMs and responder `/start`/`/answer` commands. Keep `max_connections=1` during this test. Use `/confirm` to execute a proposed booking change or `/cancel` to discard it. Bookings persist in Firestore and appear at `/bookings`; Calendar can remain disabled. No demo services or hours are added automatically.
...
Bot Settings includes an owner-only Telegram account connection. Provision `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, and `TELEGRAM_SESSION_ENCRYPTION_KEY` in Secret Manager before deploying the function. The last value is 32 cryptographically random bytes encoded as base64; preserve it across releases. Never expose these values in frontend env, build output, Git, or logs. Local development can supply the same names through the backend environment. No account is connected automatically by deployment: the owner enters their phone, Telegram login code, and optional personal Telegram account password (the password configured for two-step verification, distinct from the one-time login code) in Settings. A session can be checked and revoked there. After connection, incoming Telegram messages trigger a throttled read of the matched schedule chat: at most the latest five messages every five minutes. Imported text appears read-only on `/schedule`; the source peer ID is retained after the first match. This does not change booking availability.

## Encryption key deployment checks

`GOOGLE_CALENDAR_ENCRYPTION_KEY` is an encryption key, not an arbitrary password: generate 32 cryptographically random bytes encoded as base64 (44 characters including final `=`), for example with `openssl rand -base64 32`. Preserve the generated key across releases. An invalid value fails backend startup validation and can make every API route return 503, including `/api/health`.

`firebase:prepare` checks locally supplied Calendar and Telegram encryption keys before building (shell values override root `.env`). It never prints or packages their values. Missing local keys are allowed for deployments using existing Secret Manager versions; this local check cannot verify values entered later at a Firebase prompt. Saving a corrected key in root `.env` only fixes local configuration. Save that same value as the corresponding Secret Manager secret, then explicitly redeploy the function to bind the new version. Never replace a valid key used by existing encrypted connections without a recovery plan. No production deployment is implied by secret provisioning.

Root `.env` is not copied to production. Nonsecret runtime variables still need to be exported in the shell running `firebase:prepare`; in particular, Calendar panel setup requires `GOOGLE_CLIENT_ID` and `GOOGLE_CALENDAR_REDIRECT_URI`. Verify these separately if the API is healthy but Calendar Settings reports unconfigured.

## Private AI chat and Telegram MCP

The new `/ai-chat` page uses current owner UIDs and the Settings stakeholder email list. Listed users can access the owner's connected Telegram account through this workspace; their AI threads remain private to their own UID. It has no booking-admin navigation. Existing admin access is unchanged.

The Python MCP bridge must be provisioned separately before Telegram tools work; ordinary text chat can use existing OpenAI configuration. Source and container instructions are in `services/telegram-mcp/README.md`. Deploy that container privately to Cloud Run in the same region/project, with no unauthenticated invocation and only the API runtime service account granted `roles/run.invoker`. Set backend-only `TELEGRAM_MCP_BRIDGE_URL` to its HTTPS service origin when preparing the Firebase API release. This URL is not a frontend variable or secret. The bridge receives the current encrypted-store session after server-side decryption, only during an authorized tool call, and never persists it. Keep the existing Telegram encryption key unchanged. No additional Telegram login is needed. Confirmations send from the connected user account, not the booking bot. Do not add a Hosting rewrite or public access for this service. Deployment still requires explicit authorization for this feature.
...
Configure the existing Google OAuth **Web application** client with this authorized redirect URI:
`https://anna-ai-assistant.web.app/google-calendar/callback`.
Set backend `GOOGLE_CLIENT_ID` to the OAuth client ID, `GOOGLE_CALENDAR_REDIRECT_URI` to that exact callback URI, and `GOOGLE_CALENDAR_ACCOUNT_EMAIL=anna.lush.massage@gmail.com`. Calendar API is required. Keep `GOOGLE_CLIENT_SECRET` in Secret Manager; provision `GOOGLE_CALENDAR_ENCRYPTION_KEY` as 32 random bytes encoded in base64 and keep it stable across releases. Preparation copies these nonsecret runtime variables only; the key is bound as a function secret. Register a localhost callback URI separately for development.

Owners then use Bot Settings → Google Calendar → Connect Google Calendar, authorize Anna's Google account, select a writable calendar, and check the connection. Tokens are exchanged/encrypted on the backend. Requested scopes are `openid`, `email`, `https://www.googleapis.com/auth/calendar.events`, `https://www.googleapis.com/auth/calendar.freebusy`, and `https://www.googleapis.com/auth/calendar.calendarlist.readonly`. Configure the consent screen and add Anna as a test user if in Testing. External apps in Testing generally receive refresh tokens that expire after seven days for Calendar scopes; use the appropriate production publishing/verification process for durable access. Firebase login and Calendar authorization are separate.
