...
Schedule source configuration uses owner-only dialog discovery and verified ID selection through the existing Telegram account lease. Manual refresh and webhooks use the same persisted five-minute claim. Safe failure categories and attempt/success timestamps are returned with the read-only snapshot; attempt IDs reject stale completions after source changes.
