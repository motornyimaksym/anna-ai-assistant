...
export const createApp = async () => { const app = await NestFactory.create<NestExpressApplication>(AppModule, { logger: ['error', 'warn', 'log'], bodyParser: false }); app.useBodyParser('json', { limit: '8mb' }); app.enableCors({ origin: true }); return app; };
